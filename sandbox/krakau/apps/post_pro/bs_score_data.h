#ifndef SANDBOX_KRAKAU_APPS_POST_PRO_BS_SCORE_DATA_H_
#define SANDBOX_KRAKAU_APPS_POST_PRO_BS_SCORE_DATA_H_
...
public:
    ...
    BsSubstitutionMatrix(TValue &fMeth, TValue &fBsConversion):
        fMatch(0.9/4.0),  //(0.9/4.0),
        fSubst(0.1/12.0), //(0.1/12.0),
        fNtoX(0.02/4.0),
        fA(0.9/4.0),
        fC(0.9/4.0),
        fG(0.9/4.0),
...
public:
    ...
    BsSubstitutionMatrix()
    {
        std::cout << "Construct simple subst. matrix, CT" << std::endl;
        TValue temperature = 1;
        TValue match = exp(1.0/temperature);
        TValue mismatch = exp(-1.0/temperature);

        static TValue const tab[25] = {
                    //A, C, G, T, N (ref?)
                      match,    mismatch,   mismatch,   mismatch,   0,
                      mismatch, match,      mismatch,   match,      0,    // Ref:C - read:T       
                      mismatch, mismatch,   match,      mismatch,   0,
                      mismatch, mismatch,   mismatch,   match,      0,    
                      mismatch, mismatch,   mismatch,   mismatch,   0
        };
        arrayCopy(tab, tab + 25, (*this).data_tab);
    }
};*/
...
public:
    ...
    BsSubstitutionMatrix(TValue &fMeth, TValue &fBsConversion):
        fMatch(0.89/4.0),  //(0.9/4.0),
        fSubst(0.09/12.0), //(0.1/12.0),
        fNtoX(0.02/4.0),
        fA(0.9/4.0),
        fC(0.9/4.0),
        fG(0.9/4.0),
...
public:
    ...
    BsSubstitutionMatrix()
    {
        ...
        static TValue const tab[25] = {
                      ...
                      match,    mismatch,   match,      mismatch,   0,    // Ref:G - read:A
                      mismatch, mismatch,   mismatch,   match,      0,    
                      mismatch, mismatch,   mismatch,   mismatch,   0
        };
        arrayCopy(tab, tab + 25, (*this).data_tab);
