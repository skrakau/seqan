...
#ifndef __SANDBOX_KRAKAU_APPS_SNP_METH_STORE_SNP_METH_STORE_H__
#define __SANDBOX_KRAKAU_APPS_SNP_METH_STORE_SNP_METH_STORE_H__
...
struct MethCallingOptions
{
    ...
    bool callOnlyCSnps;
    // TODO: if true, look only at positions with C in ref
    bool betaSampling;
    bool polynomialProbFunction;
    bool logSpace;
    ...
    MethCallingOptions() : 
        ...
        minScoreToCallSnp(9),  // TODO set to 10 ?
    ...
    while (!atEnd(reader))
    {
        ...
        if (mScore >= options.minMapQual)        {
            ...
            if (beginPos < 300 || endPos < 300) 
            {
                ...
                std::cout << " beginPos: " << beginPos << "  endPos: " << endPos  << " orientation: " << orientation << std::endl;
    ...
    while(matchIt != matchItEnd)
    {
        ...
        if(numMatches >= options.minCoverage)
        {
            ...
            if(indelReadCount >= (int)options.indelCountThreshold)
            {
                ...
                doSnpAndMethCalling(fragStoreGroup, groupStartCoord, groupWindowBegin, groupWindowEnd, true, file, methOptions, options); 
            }
            else
            {
                doSnpAndMethCalling(fragStoreGroup, groupStartCoord, groupWindowBegin, groupWindowEnd, false, file, methOptions, options); 
            }
        }
    }  
}
    ...
    TOptions                &options)
{
    ...
    { 
        ...
        if (false) //candidatePos + startCoord == 1000)
        {
            ...
            {
                std::cout << (Dna)i << " " << countF[i] << std::endl;
            }
            for (unsigned i = 0; i < 4; ++i)
            {
                std::cout << (Dna)i << " " << countR[i] << std::endl;
            }
        }
        ...
        if ( (refBase == 'C' || refBase == 'G' || (observedAtLeastOneMut && positionCoverage > options.minCoverage)) && !refGap)  
        {
            ...
            if (meth.genotypeCalled || meth.bsCalled || refAllele == 'C' || refAllele == 'G') //(methOptions.outputCandidates & (refAllele == 'C' || refAllele == 'G') ))
                writeMeth(file, meth, qualityStringF, qualityStringR, refContext, positionCoverage, methOptions, options);
        }
       
        // TODO: maybe output for C positions, if potential indel ???
        bool isIndel = true;
        if ( numIndelsObserved < (int)options.indelCountThreshold) // count threshold
            isIndel = false;
